...
class KeysightEDU36311A_PowerSupply(BaseDriver):
    ...
    def get_channel_voltage(self, channel) -> float:
        """Query voltage of a given channel."""
    ...
    def set_channel_voltage(self, channel, voltage: float):
        ...
        ch = self._validate_channel(channel)
        limits = {
            1: (_USER_MIN_CH1_VOLTAGE, _USER_MAX_CH1_VOLTAGE),
            2: (_USER_MIN_CH2_VOLTAGE, _USER_MAX_CH2_VOLTAGE),
            3: (_USER_MIN_CH3_VOLTAGE, _USER_MAX_CH3_VOLTAGE),
        }[ch]
        if not (limits[0] <= voltage <= limits[1]):
    ...
    def get_channel_current(self, channel) -> float:
        """Query current of a given channel."""
        ch = self._validate_channel(channel)
        resp = self.query_check(f"APPLy? ch{ch}")
    ...
    def set_channel_current(self, channel, current: float):
        ...
        ch = self._validate_channel(channel)
        limits = {
            1: (_USER_MIN_CH1_CURRENT, _USER_MAX_CH1_CURRENT),
            2: (_USER_MIN_CH2_CURRENT, _USER_MAX_CH2_CURRENT),
            3: (_USER_MIN_CH3_CURRENT, _USER_MAX_CH3_CURRENT),
        }[ch]
    ...
    def return_instrument_parameters(self, print_output=False) -> dict:
        """Run all get_* methods and return their results."""
        return super().return_instrument_parameters(print_output)


if __name__ == '__main__':
    """Example usage for PSU_Keysight_E36311A driver."""
    import logging
    ...
    HEMT_PSU = PSU_Keysight_E36311A(cfg, debug=True)
    time.sleep(5)
